...
public class CalendarDialog extends JDialog {
    ...
    // FIELDS
    private static final Dimension dialogDim = new Dimension(450,360);
    ...
    // CONSTRUCTOR
    public CalendarDialog(MainFrame mainFrame, JTextField field) {
        super(mainFrame, "JuroVet: Calendar", true);
        this.mainFrame = mainFrame;
        Container pane = getContentPane();
        pane.setLayout(null);
        setLocation(mainFrame.getWidth()/2 - dialogDim.width/2, mainFrame.getHeight()/2 - dialogDim.height/2);
        ...
        for (String day : dayList) {
            tblCalendar.getColumn(day).setCellRenderer(new CalendarButtonRenderer());
            tblCalendar.getColumn(day).setCellEditor(new CalendarDateSelector(this, new JCheckBox(), field));
        }
        ...
        btnToday.setBounds(175, 5, 80, 30);

        // ADD CONTROLS TO PANE
        pane.add(pnlCalendar);
        pnlCalendar.add(lblMonth); pnlCalendar.add(cmbYear);
    ...
    static int getYear() { return currentYear; }
    static int getMonth() { return currentMonth; }
    ...
    private static void refreshCalendar(int month, int year){
        ...
        lblMonth.setBounds(tblCalendar.getWidth()/4-lblMonth.getPreferredSize().width/4, 0, 180, 25);
